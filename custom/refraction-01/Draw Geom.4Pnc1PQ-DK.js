  ...
  const drawCmd = {
    pipeline: ctx.pipeline({
      ...
      void main () {
        ...
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(0.0, 0.0) / uScreenSize.xy, 1.0 + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(1.0, 0.0) / uScreenSize.xy, 1.0 + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(-1.0, 0.0) / uScreenSize.xy, 1.0 + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(0.0, 1.0) / uScreenSize.xy, 1.0 + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(0.0,-1.0) / uScreenSize.xy, 1.0 + blur * 5.0);
        gl_FragData[0] /= 5.0;
