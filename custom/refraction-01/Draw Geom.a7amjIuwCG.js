module.exports = (node, graph) => {
  ...
  const roughnessIn = node.in('roughness', 0.0)
  ...
  const drawCmd = {
    pipeline: ctx.pipeline({
      ...
      void main () {        
        ...
        vPositionView = (uViewMatrix * uModelMatrix * vec4(aPosition + aOffset, 1.0)).xyz;
      ...
      void main () {
        ...
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(0.0, 0.0) / uScreenSize.xy, uRoughness + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(1.0, 0.0) / uScreenSize.xy, uRoughness + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(-1.0, 0.0) / uScreenSize.xy, uRoughness + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(0.0, 1.0) / uScreenSize.xy, uRoughness + blur * 5.0);
        gl_FragData[0] += texture2DLodEXT(uFrameColorMap, screenUV + br * vec2(0.0,-1.0) / uScreenSize.xy, uRoughness + blur * 5.0);
        gl_FragData[0] /= 5.0;
  ...
  triggerIn.onTrigger = (props) => {
    ...
    mat4.translate(modelMatrix, positionIn.value)
    ...
    ctx.submit(drawCmd, {
      uniforms: {
        ...
        uRoughness: roughnessIn.value * 6.0
      }
    })
